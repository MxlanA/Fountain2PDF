...
def create_pdf(file_path, output_file):
...
def main(page: ft.Page):
    ...
    async def choose_file(e):
        ...
        try:
            ...
            # First create the PDF in the app's temporary/private area.
            # We then export it through Android's system Save dialog so
            # the user can choose Downloads, Documents, etc.
            import tempfile
            import shutil

            temp_dir = Path(tempfile.gettempdir())
            temp_file = temp_dir / (Path(file_path).stem + ".pdf")
            ...
            save_path = await file_picker.save_file(
                file_name=Path(file_path).stem + ".pdf",
                allowed_extensions=["pdf"]
            )

            if not save_path:
                status.value = "PDF created, but saving was cancelled."
            else:
                shutil.copyfile(
                    str(temp_file),
                    str(save_path)
